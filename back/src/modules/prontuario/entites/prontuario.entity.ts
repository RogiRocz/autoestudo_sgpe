...
export class Prontuario {
    ...
    @ApiProperty({
        ...
        description: 'ID único do paciente vinculado',
    })
    paciente_id?: string | null

    @ApiProperty({
        example: 'uuid-do-aluno',
        description: 'ID único do aluno vinculado',
    })
    ...
    @ApiProperty({
        enum: TIPO_SESSAO,
        example: TIPO_SESSAO.INDIVIDUAL,
        description: 'Tipo de sessão a ser realizada',
    })
    tipo_sessao: TIPO_SESSAO

    @ApiProperty({
        enum: LOCAL_SESSAO,
        example: LOCAL_SESSAO.SALA_01,
        description: 'Local onde a sessão será realizada',
    })
    local: LOCAL_SESSAO

    @ApiProperty({
        enum: PRONTUARIO_STATUS,
        example: PRONTUARIO_STATUS.REALIZADO,
        description: 'Status no prontuário da sessão',
    })
    status: PRONTUARIO_STATUS

    @ApiProperty({
        example: 'Paciente apresentou evolução no quadro...',
    ...
    @ApiProperty({
        description: 'Data da atualização de alguma informação do aluno',
    })
    atualizadoEm: Date
