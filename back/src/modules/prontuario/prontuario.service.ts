...
export class ProntuarioService {
            ...
            if (error instanceof Prisma.PrismaClientKnownRequestError) {
                if (error.code == 'P2002') {
                    throw new NotFoundException(
                        'Valor inválido: Prontuário não encontrado'
                    )
                }
            }
    ...
    async update(
        id: string,
        dadosNovos: UpdateProntuarioDTO
    ): Promise<Prontuario> {
        try {
            ...
            if (prontuarioAntigo.paciente_id != dadosNovos.paciente_id) {
                throw new ConflictException(
                    'Falha ao atualizar: O prontuário não pode ser mudado de paciente'
                )
            }

            if (prontuarioAntigo.aluno_id != dadosNovos.aluno_id) {
                throw new ConflictException(
                    'Falha ao atualizar: O prontuário não pode ser mudado de aluno'
                )
            }
