...
import {
    ...
    UserEntityMap,
    UserType,
} from '../../common/interfaces/IAuth.interface'
...
export class AuthService {
    ...
    getService(nameType: UserType): IAuthService {
        if (!Object.values(UserType).includes(nameType)) {
            throw new BadRequestException(
                'Falha na requisição: tipo de usário inválido'
            )
        }
    ...
    async getUser<T extends UserEntityMap[UserType]>(
        payload: UserPayload
    ): Promise<T> {
    ...
    async verifyPassword<T extends IAuthenticatable>(
        user: T,
        plainPass: string
    ): Promise<boolean> {
        ...
        if (!result) {
            throw new UnauthorizedException(
                'Falha no login: Usuário não autorizado'
            )
        }
    ...
    async createToken<T extends IAuthenticatable>(
        user: T,
        type: UserType
    ): Promise<string> {
        try {
            ...
            throw new InternalServerErrorException(
                'Falha na criação do token: Erro: ' + error
            )
        }
    }
    ...
    async registerUser<K extends UserType>(
        type: K,
        dados: CreateDTOMap[K]
    ): Promise<AuthResponse<K>> {
        const service = this.getService(type) as IAuthService<
            CreateDTOMap[K],
            UserEntityMap[K]
        >
        ...
        if (!dados.senha) {
            throw new BadRequestException(
                'Falha no registro: Senha não fornecida do DTO'
            )
        }
    ...
    async loginUser<K extends UserType>(
        type: K,
        credenciais: Omit<LoginDTO, 'type'>
    ): Promise<AuthResponse<K>> {
        try {
            const service = this.getService(type)
            const userDB = (await service.findByIdentifier(
                credenciais.login,
                credenciais.field
            )) as UserEntityMap[K] | null

            if (!userDB) {
                throw new UnauthorizedException(
                    'Falha no login: Usuário do banco de dados não encontrado'
                )
            }
