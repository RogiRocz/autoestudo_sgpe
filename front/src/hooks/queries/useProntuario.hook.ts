...
export const useProntuarios = (params?: QueryParams) => {
    ...
    return useQuery({
        ...
        queryFn: () => ProntuarioService.getProntuarios(user!.type, user!.uuid, params),
        enabled: isAuthenticated
    })
}

export const useProntuariosByLocal = (local: LOCAL_SESSAO, params?: QueryParams) => {
    const {user, isAuthenticated} = useAuthUser()
    const owner: OwnerData = { tipoDono: MAP_TIPO_DONO[user!.type], donoId: user!.uuid}
...
export const useProntuariosByStatus = (status: PRONTUARIO_STATUS, params?: QueryParams) => {
    const { user, isAuthenticated } = useAuthUser()
    const owner: OwnerData = { tipoDono: MAP_TIPO_DONO[user!.type], donoId: user!.uuid }
...
export const useProntuariosByTime = (dates: SearchDate, params?: QueryParams) => {
    const { user, isAuthenticated } = useAuthUser()
    const owner: OwnerData = { tipoDono: MAP_TIPO_DONO[user!.type], donoId: user!.uuid }
